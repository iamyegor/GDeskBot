...
public class UnbanUserCommand : IBotCommand
{
    ...
    {
        var command = new Application.Commands.UnbanUserCommand(request.Text);
        ...
        if (userOrError.IsSuccess)
        {
            await _client.SendTextMessageAsync(
                ...
                text: "\ud83d\udfe2 The user has been unbanned.\n" + 
                      "\ud83d\udfe2 Пользователь был разблокирован.",
                cancellationToken: ct
            );

            await _client.SendTextMessageAsync(
                chatId: userOrError.Value.TelegramId,
                text: "\ud83d\udca5 You has been unbanned by the admin.\n"
        ...
        else
        {
            await _client.SendTextMessageAsync(
                chatId: request.Request.Message!.Chat.Id,
                text: "\ud83e\udd37\u200d\u2640\ufe0f The user you were trying to unban either doesn't exist or is not banned.\n"
